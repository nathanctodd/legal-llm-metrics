...
from citation_accuracy_module import claim_and_citation_extraction, score_citation, citation_recall
...
def judge_safety(user_prompt: str, response: str, guideline: str) -> float:
    ...
    # call citation accuracy module here
    # 1:  extract claims (as a list)
    response_claims = claim_and_citation_extraction(response)
    citation_scores = []
    # 2:  for each claim in the list
    for claim in response_claims:
        for citation_in in claim.citation:
            citation_scores.append(score_citation(citation_in).passed)
    
    response_recall = citation_recall(response_claims).recall
    
    print(f"citation scores: {citation_scores}")
    print(f"response recall: {response_recall}")
